...
http.createServer(function(request, response){
	...
	if( request.method == "POST" && file == "/enviar" ){
		...
		request.on("data", function(body){
			...
			fs.readFile(__dirname + '/public/resultado.html', function(error, content){

				content = content.toString()
				content = content.replace('@@TITULO@@', datos.titulo)
				content = content.replace('@@AUTOR@@', datos.autor)
				content = content.replace('@@DETALLE@@', datos.detalle)
